...
export function LogoIcon({ className = "h-9 w-auto", useImage = false }) {
  ...
  return (
    ...
    >
      <defs>
        {/* Navy/Slate Gradient for F-Stem */}
...
export default function Logo({ showText = true, size = "md", useImage = true, className = "" }) {
  const sizeClasses = {
    sm: "h-7",
    md: "h-9",
    lg: "h-12",
  ...
  return (
    <div className={`flex items-center gap-2.5 ${className}`}>
      <img
        src="/logoidea.jpeg"
        alt="FundX"
        className={`${iconClass} object-contain mix-blend-multiply dark:mix-blend-normal rounded-md`}
      />
      {showText && (
        <div className="flex flex-col">
          <span className={`font-black tracking-tight ${textSizeClasses[size] || textSizeClasses.md} font-poppins text-slate-900 dark:text-blue leading-none`}>
            FUND<span className="text-emerald-500">X</span>
          </span>
